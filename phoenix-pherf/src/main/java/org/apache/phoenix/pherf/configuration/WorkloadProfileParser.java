...
public class WorkloadProfileParser {

    private static final Logger LOGGER = LoggerFactory.getLogger(WorkloadProfileParser.class);
    private String filePattern;
    private List<WorkloadProfile> profiles;
    private ResourceList resourceList;
    private Collection<Path> paths = null;

    public WorkloadProfileParser(String pattern) throws Exception {
        init(pattern);
    }
    ...
    public static WorkloadProfile readWorkloadProfile(Path file) throws JAXBException, XMLStreamException {
        ...
        String fName = PherfConstants.RESOURCE_PROFILE + "/" + file.getFileName().toString();
        LOGGER.info("Open config file: " + fName);
        XMLStreamReader xmlReader = xif.createXMLStreamReader(
            new StreamSource(WorkloadProfileParser.class.getResourceAsStream(fName)));
        return (WorkloadProfile) jaxbUnmarshaller.unmarshal(xmlReader);
    }
    ...
    public static void writeWorkloadProfile(WorkloadProfile profile, OutputStream output) throws JAXBException {
        ...
        // Writing to console
        jaxbMarshaller.marshal(profile, output);
    }
    ...
    private void init(String pattern) throws Exception {
        ...
        this.resourceList = new ResourceList(PherfConstants.RESOURCE_PROFILE);
