...
Contributors: [Kadir Ozdemir](mailto:kozdemir@salesforce.com) [Andrew Purtell](mailto:apurtell@salesforce.com)  
Updated: Mar 3, 2025
...
The main approach for this zero data loss failover is to synchronously replicate the data changes to a set of HDFS files in the standby cluster and then apply these changes to the Phoenix standby cluster asynchronously as previously sketched in the [Nex Gen Sor with HBase on S3](https://docs.google.com/document/d/1vQj1qfuhWWJHbOYJd5k_qe9iEINoZzhoA-bQxTaBgKU/edit?pli=1&tab=t.0).This document provides the optimal design for this approach for Phoenix.
...
1. Synchronous replication for unverified index mutations, data table mutations and verified index mutation together in parallel with primary cluster WAL appends for index unverified index row writes  
2. Primary cluster WAL append for the data table row writes  
3. Primary cluster WAL append for verified index row writes
...
This section of this document is not updated any more. Please see [the open source version of the document](https://docs.google.com/document/d/1U2ULZ1xENDxGhAxdy5U0JYGVO5790VR5AOAJMiXHIPc/edit?tab=t.0#heading=h.pfjicxriktfm) for the up-to-date version of the design details.
...
3. **HA Group Store Client (on servers)**, which runs on the server side inside the HBase Master and Regionservers, listens to the HA Group Store for updates on the HA group records, and also updates the HA group records.   
