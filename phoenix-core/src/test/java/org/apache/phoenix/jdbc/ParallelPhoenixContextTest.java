...
public class ParallelPhoenixContextTest {
    ...
    List<ExecutorService> executorList;

    @Before
    public void init() {
        executorList =
                Lists.newArrayList(new TrackingThreadPoolExecutor(),
        ...
        try {
            ParallelPhoenixContext context =
                    new ParallelPhoenixContext(new Properties(),
                            ...
                            Lists.newArrayList(Executors.newSingleThreadExecutor()), null);
    ...
    public void testPool1OutOfCapacity() throws Exception {
        ...
        assertEquals(0, ((TrackingThreadPoolExecutor) executorList.get(0)).tasksExecuted.get());
        assertEquals(0, ((TrackingThreadPoolExecutor) executorList.get(1)).tasksExecuted.get());
        CompletableFuture<Boolean> future2 = context.chainOnConn2(() -> true);
        assertTrue(future2.get());
        assertEquals(0, ((TrackingThreadPoolExecutor) executorList.get(0)).tasksExecuted.get());
        assertEquals(1, ((TrackingThreadPoolExecutor) executorList.get(1)).tasksExecuted.get());
    }
    ...
    public void testPool2OutOfCapacity() throws Exception {
        ...
        assertEquals(1, ((TrackingThreadPoolExecutor) executorList.get(0)).tasksExecuted.get());
        assertEquals(0, ((TrackingThreadPoolExecutor) executorList.get(1)).tasksExecuted.get());
        CompletableFuture<Boolean> future2 = context.chainOnConn2(() -> true);
