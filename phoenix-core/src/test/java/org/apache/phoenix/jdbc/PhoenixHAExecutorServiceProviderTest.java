...
public class PhoenixHAExecutorServiceProviderTest {
    ...
    public void afterTest() {
        for (ExecutorService es : PhoenixHAExecutorServiceProvider.get(properties)) {
            es.shutdownNow();
        };
    }
    ...
    private void testHAExecutorServiceCapacity(int index) {
        ...
        ThreadPoolExecutor es =
                (ThreadPoolExecutor) PhoenixHAExecutorServiceProvider.get(properties).get(index);
    ...
    public void testHAExecutorServiceQueuing() {
        ThreadPoolExecutor es =
                (ThreadPoolExecutor) PhoenixHAExecutorServiceProvider.get(properties).get(0);
        Object obj = new Object();
        CompletableFuture.runAsync(getWaitingRunnable(obj), es);
        assertEquals(es.getQueue().size(), 0);
        CompletableFuture.runAsync(getWaitingRunnable(obj), es);
