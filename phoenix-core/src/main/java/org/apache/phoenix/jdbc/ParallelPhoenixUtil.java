...
public class ParallelPhoenixUtil {
    ...
    public Object getAnyOfNonExceptionally(List<CompletableFuture<? extends Object>> futures, ParallelPhoenixContext context) throws SQLException {
        ...
        while (!futures.isEmpty() && result == null) {
             ...
             */
            if(changed) {
                future = CompletableFuture.anyOf(futures.toArray(new CompletableFuture[0]));
            }
            try {
                result = future.get(internalTimeoutMs, TimeUnit.MILLISECONDS);
                break;
            } catch (Exception e) {
        ...
        if (futures.isEmpty()) {
            ...
            for(CompletableFuture<?> failedFuture : originalFutures) {
                try {
                    ...
                    LOGGER.error("Future Exception. HAGroup:" + context.getHaGroup(), e);
                ...
                if(failedFuture.isCompletedExceptionally()) {
                    try {
                        ...
                        if (LOGGER.isDebugEnabled()) {
                            LOGGER.debug("Future Exception. HAGroup:" + context.getHaGroup(), e);
                        }
                    }
                }
