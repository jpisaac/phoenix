...
/**
 * Salesforce-specific implementation of the Phoenix driver which has logging capabilities.
 * We can use this to log per-request metrics. This class is basically the same as PhoenixDriver,
 * but provides a {@link SfdcLoggingPhoenixConnection} instead of a regular {@link PhoenixConnection}
 */
public final class SfdcLoggingPhoenixDriver extends PhoenixEmbeddedDriver {

    private static final Logger LOGGER = LoggerFactory.getLogger(SfdcLoggingPhoenixDriver.class);
    public static final SfdcLoggingPhoenixDriver INSTANCE;

    private static volatile String driverShutdownMsg;
    static {
        try {
            INSTANCE = new SfdcLoggingPhoenixDriver();
            try {
                ...
                LOGGER.warn("Failed to register SfdcLoggingPhoenixDriver shutdown hook as the JVM is already shutting down");
            ...
            throw new IllegalStateException("Unable to register " + SfdcLoggingPhoenixDriver.class.getName() + ": "+ e.getMessage());
        }
    }

    private static void closeInstance(SfdcLoggingPhoenixDriver instance) {
        try {
            instance.close();
        } catch (SQLException e) {
            LOGGER.warn("Unable to close SfdcLoggingPhoenixDriver on shutdown", e);
        } finally {
            driverShutdownMsg = "SfdcLoggingPhoenixDriver closed because server is shutting down";
        }
    }
    ...
    public SfdcLoggingPhoenixDriver() { // for Squirrel
    ...
    // Override the base implementation to provide a SfdcLoggingPhoenixConnection object
    @Override
    protected final Connection createConnection(String url, Properties info) throws SQLException {
        LOGGER.info(String.format("Creating Connection with connectionString, %s", url));
        ...
        PhoenixConnection phxConnection = connectionServices.connect(url, augmentedInfo);
        PhoenixMetricsLog phoenixMetricsLog = new SfdcPhoenixMetricsLog();
        return new SfdcLoggingPhoenixConnection(phxConnection, phoenixMetricsLog);
    }
