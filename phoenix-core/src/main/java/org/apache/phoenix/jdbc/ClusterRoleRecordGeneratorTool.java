...
public class ClusterRoleRecordGeneratorTool extends Configured implements Tool {
    ...
    private static String getPeerZkUrl(Configuration conf, String haGroupName) throws IOException {
        ...
        return getPeerClusterKey(new ReplicationAdmin(conf), peerId);
    }
     ...
     */
    @VisibleForTesting
    static String getPeerClusterKey(ReplicationAdmin replicationAdmin, String id)
            throws IOException {
        ReplicationPeerConfig replicationConfig;
        try {
            replicationConfig = replicationAdmin.getPeerConfig(id);
        } catch (ReplicationException re) {
            String msg = "Can not get replication peer (id=" + id + ") config";
            LOG.error(msg, re);
            throw new IOException(msg, re);
        }
