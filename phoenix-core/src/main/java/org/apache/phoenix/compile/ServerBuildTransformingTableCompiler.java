...
public class ServerBuildTransformingTableCompiler extends ServerBuildIndexCompiler {
    ...
    public MutationPlan compile(PTable newTable) throws SQLException {
        try (final PhoenixStatement statement = new PhoenixStatement(connection)) {
            ...
            // However, in this case, we need to project all of the data columns that contribute to the transfroming table.
            if (newTable.getImmutableStorageScheme() == PTable.ImmutableStorageScheme.SINGLE_CELL_ARRAY_WITH_OFFSETS) {
                for (PColumnFamily family : dataTable.getColumnFamilies()) {
                    scan.addFamily(family.getName().getBytes());
                }
            } else {
                for (PColumn column : newTable.getColumns()) {
                    byte[] family = null;
                    if (column.getFamilyName() == null) {
                        family = new ImmutableBytesPtr().copyBytesIfNecessary();
                    } else {
                        family = column.getFamilyName().getBytes();
                    }
                    scan.addColumn(family, column.getColumnQualifierBytes());
                }
            }
