...
public class ViewTTLIT extends BaseViewTTLIT {
  ...
  public void testPhoenixTTLWithTableLevelTTLFails() throws Exception {
    ...
    tableOptions.setTableProps(String
      .format("COLUMN_ENCODED_BYTES=0,MULTI_TENANT=true," + "TTL='%s'", getTTLExpression(100)));

    TenantViewOptions tenantViewOptions = TenantViewOptions.withDefaults();
    tenantViewOptions.setTableProps(String.format("TTL='%s'", getTTLExpression(1000)));
    try {
  ...
  public void testPhoenixTTLWithViewIndexFails() throws Exception {
    ...
    tenantViewIndexOptions.setIndexProps(String.format("TTL='%s'", getTTLExpression(1000)));
    try {
      final SchemaBuilder schemaBuilder = createLevel1TenantView(null, tenantViewIndexOptions);
      fail();
    } catch (SQLException e) {
  ...
  public void testPhoenixTTLForLevelOneView() throws Exception {
    ...
    String ttlExpr = getTTLExpression(VIEW_TTL_120_SECS);
    ...
    assertSyscatHavePhoenixTTLRelatedColumns(tenantId, schemaName, tenantViewName,
      PTableType.VIEW.getSerializedValue(), ttlExpr);
  ...
  public void testPhoenixTTLForLevelTwoView() throws Exception {
    ...
    final SchemaBuilder schemaBuilder = createLevel2TenantViewWithGlobalLevelTTL(null, null, false);
    ...
    String ttlExpr = getTTLExpression(300);
  ...
  public void testPhoenixTTLWithAlterView() throws Exception {
    ...
    try (Connection connection = DriverManager.getConnection(tenantURL)) {
      try (Statement stmt = connection.createStatement()) {
        // View TTL is set to 120s => 120000 ms
        String sql =
          String.format(ALTER_TTL_SQL, schemaName, tenantViewName, getTTLExpression(ttl));
        stmt.execute(sql);
      }
    }
    ...
    assertViewHeaderRowsHavePhoenixTTLRelatedCells(schemaBuilder.getTableOptions().getSchemaName(),
      startTime, false, 1);
    assertSyscatHavePhoenixTTLRelatedColumns(tenantId, schemaName, tenantViewName,
      PTableType.VIEW.getSerializedValue(), getTTLExpression(ttl));
    assertSyscatHavePhoenixTTLRelatedColumns(tenantId, schemaName, indexOnTenantViewName,
      PTableType.INDEX.getSerializedValue(), "0");
  ...
  public void testTTLAlreadyDefinedInHierarchyWhenCreateView() throws Exception {
    try {
      ...
      tenantViewWithOverrideOptions
        .setTableProps(String.format("TTL='%s'", getTTLExpression(VIEW_TTL_120_SECS)));
      final SchemaBuilder schemaBuilder =
        createLevel2TenantViewWithGlobalLevelTTL(tenantViewWithOverrideOptions, null, false);
      fail();
    } catch (SQLException e) {
      assertEquals(SQLExceptionCode.TTL_ALREADY_DEFINED_IN_HIERARCHY.getErrorCode(),
        e.getErrorCode());
  ...
  public void testTTLAlreadyDefinedInHierarchyWhenAlterTenantView() throws Exception {
    ...
    try (Connection connection = DriverManager.getConnection(tenantURL)) {
      try (Statement stmt = connection.createStatement()) {
        // View TTL is set to 120s => 120000 ms
        String sql = String.format(ALTER_TTL_SQL, schemaName, tenantViewName, "120");
        stmt.execute(sql);
        fail();
      }
    } catch (SQLException e) {
  ...
  public void testTTLAlreadyDefinedInHierarchyWhenAlterGlobalView() throws Exception {
    ...
    try (Connection connection = DriverManager.getConnection(getUrl())) {
      try (Statement stmt = connection.createStatement()) {
        // View TTL is set to 120s => 120000 ms
        String sql =
          String.format(ALTER_TTL_SQL, schemaName, globalViewName, getTTLExpression(120));
        stmt.execute(sql);
        fail();
      }
    } catch (SQLException e) {
  ...
  public void testAlterViewWithNoTTLPropertySucceed() throws Exception {
    ...
    try (Connection connection = DriverManager.getConnection(getUrl())) {
      try (Statement stmt = connection.createStatement()) {
        String sql = String.format(ALTER_SQL_WITH_NO_TTL, schemaName, globalViewName, "COL_30");
        stmt.execute(sql);
      }
    }
    ...
    try (Connection connection = DriverManager.getConnection(tenantURL)) {
      try (Statement stmt = connection.createStatement()) {
        String sql = String.format(ALTER_SQL_WITH_NO_TTL, schemaName, tenantViewName, "COL_100");
        stmt.execute(sql);
      }
    }
  ...
  public void testResetViewTTL() throws Exception {
    ...
    try (Connection connection = DriverManager.getConnection(getUrl())) {
      try (Statement stmt = connection.createStatement()) {
        ...
        String sql = String.format(ALTER_TTL_SQL, schemaName, globalViewName, "NONE");
        stmt.execute(sql);
      }
    }
  ...
  public void testWithTenantViewAndNoGlobalView() throws Exception {
    ...
    for (boolean isMultiTenant : Lists.newArrayList(true, false)) {
      ...
      DataOptions dataOptions =
        isMultiTenant ? DataOptions.withDefaults() : DataOptions.withPrefix("CUSTOM_OBJECT");

      // OID, KP for non multi-tenanted views
      int viewCounter = 1;
      String orgId = String.format(PhoenixTestBuilder.DDLDefaults.DEFAULT_ALT_TENANT_ID_FMT,
        viewCounter, dataOptions.getUniqueName());
      ...
      if (isMultiTenant) {
        ...
        schemaBuilder.withTableOptions(tableOptions).withTenantViewOptions(tenantViewOptions)
          .withDataOptions(dataOptions).withTenantViewIndexDefaults().buildWithNewTenant();
      } else {
        ...
        globalViewIndexOptions
          .setGlobalViewIndexColumns(Lists.newArrayList(TENANT_VIEW_INDEX_COLUMNS));
        globalViewIndexOptions
          .setGlobalViewIncludeColumns(Lists.newArrayList(TENANT_VIEW_INCLUDE_COLUMNS));

        globalViewOptions.setGlobalViewCondition(
          String.format("SELECT * FROM %s.%s WHERE OID = '%s' AND KP = '%s'",
        ...
        schemaBuilder.withTableOptions(tableOptions).withGlobalViewOptions(globalViewOptions)
          .withDataOptions(dataOptions).withConnectOptions(connectOptions)
          .withGlobalViewIndexOptions(globalViewIndexOptions).build();
      }
      ...
      List<String> rowKeyColumns =
        isMultiTenant ? Lists.newArrayList("ZID") : Lists.newArrayList("OID", "KP", "ZID");
      ...
      try (Connection writeConnection = DriverManager.getConnection(tenantConnectUrl)) {
        ...
        dataWriter.setTargetEntity(isMultiTenant
        ...
        dataReader.setDML(String.format("SELECT %s from %s", Joiner.on(",").join(columns),
          isMultiTenant
            ? schemaBuilder.getEntityTenantViewName()
            : schemaBuilder.getEntityGlobalViewName()));
        dataReader.setTargetEntity(isMultiTenant
          ? schemaBuilder.getEntityTenantViewName()
          : schemaBuilder.getEntityGlobalViewName());
  ...
  public void testWithSQLUsingIndexWithCoveredColsUpdates() throws Exception {
    ...
    testCaseWhenAllCFMatchAndAllDefault.setTableCFs(Lists.newArrayList((String) null, null, null));
    ...
    for (boolean isGlobalIndexLocal : Lists.newArrayList(true, false)) {
      ...
      final SchemaBuilder schemaBuilder = new SchemaBuilder(getUrl());
      schemaBuilder.withTableOptions(tableOptions).withGlobalViewOptions(globalViewOptions)
        .withGlobalViewIndexOptions(globalViewIndexOptions).withTenantViewOptions(tenantViewOptions)
      ...
      DataSupplier dataSupplier = new DataSupplier() {
        ...
        public List<Object> getValues(int rowIndex) {
          ...
          return Lists.newArrayList(new Object[] { id, zid, col4, col5, col6, col7, col8, col9 });
        }
      };
  ...
  public void testWithSQLUsingIndexAndMultiLevelViews() throws Exception {
    ...
    String level3ViewName = String.format("%s.%s", DEFAULT_SCHEMA_NAME, "E11");
    String level3ViewCreateSQL = String.format("CREATE VIEW IF NOT EXISTS %s AS SELECT * FROM %s",
      level3ViewName, schemaBuilder.getEntityTenantViewName());
    String tConnectUrl =
      getUrl() + ';' + TENANT_ID_ATTRIB + '=' + schemaBuilder.getDataOptions().getTenantId();
    ...
    try (Connection writeConnection = DriverManager.getConnection(tenantConnectUrl)) {
      ...
      dataReader.setRowKeyColumns(rowKeyColumns);
      dataReader.setDML(
        String.format("SELECT col6 from %s where col4 = '%s'", level3ViewName, outerCol4s.get(1)));
      ...
      dataReader.setValidationColumns(rowKeyColumns1);
      dataReader.setRowKeyColumns(rowKeyColumns1);
      dataReader.setDML(String.format("SELECT id, col6 from %s where col4 = '%s'", level3ViewName,
        outerCol4s.get(1)));
  ...
  public void testWithVariousSQLs() throws Exception {
    ...
    DataSupplier dataSupplier = new DataSupplier() {
      ...
      public List<Object> getValues(int rowIndex) {
        ...
        return Lists
          .newArrayList(new Object[] { groupById, zid, col4, col5, col6, col7, col8, col9 });
      }
    };
    ...
    try (Connection writeConnection = DriverManager.getConnection(tenantConnectUrl)) {
      ...
      dataReader.setRowKeyColumns(Arrays.asList("num_rows"));
      dataReader
        .setDML(String.format("SELECT count(1) as num_rows from %s GROUP BY ID HAVING count(1) > 0",
  ...
  public void testWithVariousSQLsForMultipleTenants() throws Exception {
    ...
    for (int tenant : Arrays.asList(new Integer[] { 1, 2, 3 })) {
      ...
      try (Connection writeConnection = DriverManager.getConnection(tenantConnectUrl)) {
        ...
        dataReader.setRowKeyColumns(Arrays.asList("num_rows"));
        dataReader.setDML(
          String.format("SELECT count(1) as num_rows from %s GROUP BY ID HAVING count(1) > 0",
            schemaBuilder.getEntityTenantViewName()));
  ...
  public void testWithVariousSQLsForMultipleViews() throws Exception {
    ...
    for (int view : Arrays.asList(new Integer[] { 1, 2, 3 })) {
      ...
      try (Connection writeConnection = DriverManager.getConnection(tenantConnectUrl)) {
        ...
        dataReader.setRowKeyColumns(Arrays.asList("num_rows"));
        dataReader.setDML(
          String.format("SELECT count(1) as num_rows from %s GROUP BY ZID HAVING count(1) > 0",
            schemaBuilder.getEntityTenantViewName()));
  ...
  /**
   * ************************************************************ Case: Build schema with TTL set by
   * the tenant view. TTL for GLOBAL_VIEW - 300000ms (not set) TTL for TENANT_VIEW - 300000ms
   * ************************************************************
   */
  ...
  public void testGlobalAndTenantViewTTLInheritance1() throws Exception {
    ...
    tenantViewWithOverrideOptions
      .setTableProps(String.format("TTL='%s'", getTTLExpression(tenantPhoenixTTL)));
    ...
    /**
     * ************************************************************ Build schema with TTL set by the
     * tenant view. ************************************************************
     */
    ...
    DataSupplier dataSupplier = new DataSupplier() {
      ...
      public List<Object> getValues(int rowIndex) {
        ...
        return Lists.newArrayList(
          new Object[] { id, zid, col1, col2, col3, col4, col5, col6, col7, col8, col9 });
      }
    };

    // Create a test data reader/writer for the above schema.
    DataWriter dataWriter = new BasicDataWriter();
    List<String> columns = Lists.newArrayList("ID", "ZID", "COL1", "COL2", "COL3", "COL4", "COL5",
      "COL6", "COL7", "COL8", "COL9");
    List<String> rowKeyColumns = Lists.newArrayList("ID", "ZID");
    ...
    try (Connection writeConnection = DriverManager.getConnection(tenant1ConnectUrl)) {
      ...
      long probeTimestamp =
        EnvironmentEdgeManager.currentTimeMillis() + ((tenantPhoenixTTL * 1000) / 2);
  ...
  /**
   * ************************************************************ Case: Build schema with TTL set by
   * the global view. TTL for GLOBAL_VIEW - 300000ms TTL for TENANT_VIEW - 300000ms (not set, uses
   * global view ttl) ************************************************************
   */
  ...
  public void testGlobalAndTenantViewTTLInheritance2() throws Exception {
    ...
    int globalViewTTL = VIEW_TTL_300_SECS;
    ...
    SchemaBuilder.GlobalViewOptions globalViewOptions =
      SchemaBuilder.GlobalViewOptions.withDefaults();
    globalViewOptions.setTableProps(String.format("TTL='%s'", getTTLExpression(globalViewTTL)));
    ...
    /**
     * ************************************************************ Case: Build schema with TTL set
     * by the global view. ************************************************************
     */
    ...
    try (Connection writeConnection = DriverManager.getConnection(tenant1ConnectUrl)) {
      ...
      // Validate data exists before ttl expiration.
      long probeTimestamp =
        EnvironmentEdgeManager.currentTimeMillis() + ((globalViewTTL * 1000) / 2);
      validateRowsAreNotMaskedUsingCounts(probeTimestamp, dataReader, schemaBuilder);
      // Validate data before and after ttl expiration.
  ...
  public void testScanAttributes() throws Exception {
    ...
    TenantViewOptions tenantViewOptions = TenantViewOptions.withDefaults();
    String ttlExpr = getTTLExpression(phoenixTTL);
    tenantViewOptions.setTableProps(String.format("TTL='%s'", ttlExpr));
    ...
    try (
      PhoenixConnection conn =
        (PhoenixConnection) DriverManager.getConnection(tenantConnectUrl, props);
      ...
      PhoenixResultSet rs =
        pstmt.newResultSet(queryPlan.iterator(), queryPlan.getProjector(), queryPlan.getContext());
